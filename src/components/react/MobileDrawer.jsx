  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center justify-center w-10 h-10 rounded-lg text-ink-700 hover:bg-paper-200"
      >
      ...
      {open && (
        <div className="fixed inset-0 z-50">
          ...
          <div className="absolute right-0 top-0 h-full w-[86%] max-w-sm bg-surface shadow-xl flex flex-col">
            <div className="flex items-center justify-between px-4 h-16 border-b border-brand-200">
              ...
              <button aria-label="Cerrar menú" onClick={() => setOpen(false)} className="w-10 h-10 inline-flex items-center justify-center rounded-lg hover:bg-paper-200">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <nav className="flex-1 overflow-y-auto px-4 py-5 space-y-8">
                      ...
                      <ul className="space-y-2">
                        {col.links.map((l) => (
                          <li key={l.href}>
                            <a
                              href={l.href}
                              className={l.more ? "text-brand-700 font-medium" : "text-ink-700 hover:text-brand-700"}
                            >
                              {l.name}{l.more ? " →" : ""}
                            </a>
                          </li>
              ...
              <div className="pt-2 border-t border-line">
                <ul className="space-y-2">
                  {links.map((l) => (
                    <li key={l.href}>
                      <a href={l.href} className="font-semibold text-ink-700 hover:text-brand-700">{l.name}</a>
                    </li>
                  ))}
                </ul>
              </div>
